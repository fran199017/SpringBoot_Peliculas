...
public class RestService {
				...
				do {
					...
					for (int i = 0; i < arr.length(); i++)
					{
					    ...
					    if(original_title.contains("'") ||overview.contains("'") ) {
					    	...
					    	jdbc.execute("insert into peliculas(nombre,overview,vote_average)values('"+original_title+"',"+"'"+overview+"',"+"'"+vote_average+"')");
					    }else {
					    	jdbc.execute("insert into peliculas(nombre,overview,vote_average)values('"+original_title+"',"+"'"+overview+"',"+"'"+vote_average+"')");
				    	}
	...
	public void setActores(JdbcTemplate jdbc) {
		...
		}while(contadorActores<15);  //maximo de paginas de la api (He colocado 5 para no tardar tanto, son 982)*/		
	}
